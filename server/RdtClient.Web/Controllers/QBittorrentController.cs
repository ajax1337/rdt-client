...
public class QBittorrentController(ILogger<QBittorrentController> logger, QBittorrent qBittorrent) : Controller
{
    ...
    public async Task<ActionResult<IList<TorrentInfo>>> TorrentsInfo([FromQuery] QBTorrentsInfoRequest request)
    {
        ...
        // Filter by hashes (pipe-separated)
    ...
    [HttpGet]
    public async Task<ActionResult<IList<TorrentTracker>>> TorrentsTrackers([FromQuery] QBTorrentsHashRequest request)
    {
        if (String.IsNullOrWhiteSpace(request.Hash))
        {
            return BadRequest();
        ...
        var result = await qBittorrent.TorrentTrackers(request.Hash);
    ...
    [Route("torrents/trackers")]
    [HttpPost]
    public async Task<ActionResult<IList<TorrentTracker>>> TorrentsTrackersPost([FromForm] QBTorrentsHashRequest request)
    {
