...
public class QBittorrent(ILogger<QBittorrent> logger, Settings settings, Authentication authentication, Torrents torrents, Downloads downloads)
{
    ...
    public async Task<IList<TorrentTracker>?> TorrentTrackers(String hash)
    {
        var torrent = await torrents.GetByHash(hash);
        ...
        var results = new List<TorrentTracker>
        {
            // Add a few generic public trackers for compatibility
            new()
            {
                Url = "udp://tracker.opentrackr.org:1337/announce",
                Status = 2, // Working
                Tier = 0,
                NumPeers = torrent.RdSeeders ?? 10,
                NumSeeds = torrent.RdSeeders ?? 10,
                NumLeeches = 0,
                NumDownloaded = 100,
                Msg = "Working"
            },
            new()
            {
                Url = "udp://open.tracker.cl:1337/announce",
                Status = 2, // Working
                Tier = 1,
                NumPeers = torrent.RdSeeders ?? 5,
                NumSeeds = torrent.RdSeeders ?? 5,
                NumLeeches = 0,
                NumDownloaded = 50,
                Msg = "Working"
            },
            new()
            {
                ...
                Status = 2, // Working
                Tier = -1,
                NumPeers = torrent.RdSeeders ?? 20,
                NumSeeds = torrent.RdSeeders ?? 20,
                NumLeeches = 0,
                NumDownloaded = 0,
                Msg = ""
            },
